...
public class CarameAlbumActivity extends BaseActivity implements EasyPermissions.PermissionCallbacks {
    ...
    private void openOrClosePop() {
        carameAlbumLayout = LayoutInflater.from(CarameAlbumActivity.this).inflate(R.layout.layout_carame_ablum, null);
        ...
        mLayoutCarame.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (EasyPermissions.hasPermissions(CarameAlbumActivity.this, needShootPermissionPerms)) {
                    takeCarame();
                } else {
                    EasyPermissions.requestPermissions(CarameAlbumActivity.this, "拍照需要摄像头权限", REQUEST_CAMERA, needShootPermissionPerms);
                }
            }
        });
        mLayoutAlbum.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (EasyPermissions.hasPermissions(CarameAlbumActivity.this, needShootPermissionPerms)) {
                    ...
                    EasyPermissions.requestPermissions(CarameAlbumActivity.this, "相册需要读取权限", REQUEST_ALBUM, needShootPermissionPerms);
                }
            }
        });
    ...
    private void takeCarame() {
        ...
        Log.i("onPermissionsGranted", "getFilesDir : " + CarameAlbumActivity.this.getFilesDir());
    }

    // 设置显示的透明度 1.0 - 0.0
    public void setAlpha(float alpha) {
        WindowManager.LayoutParams attributes = CarameAlbumActivity.this.getWindow().getAttributes();
        attributes.alpha = alpha;
        CarameAlbumActivity.this.getWindow().setAttributes(attributes);
    }
        ...
        switch (requestCode) {
            case PHOTO_GRAPH: // 拍照的回调
                ...
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    Log.i("bitmap:", "" + bitmap);
                    if (isShowZheng) {
//                        Glide.with(CarameAlbumActivity.this).load(file.toString()).placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher).into(shenfenzheng);
                    } else {
//                        Glide.with(CarameAlbumActivity.this).load(file.toString()).placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher).into(shenfenfan);
                    }
                    // 上传
            ...
            case PHOTO_ALBUM:
                if (resultCode == Activity.RESULT_OK) {
                    ...
                    Cursor cursor = CarameAlbumActivity.this.managedQuery(selectedImage, proj, null, null, null);
                    if (cursor != null) {
                        ...
                        if (isShowZheng) {
//                            Glide.with(CarameAlbumActivity.this).load(imagePath).placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher).into(shenfenzheng);
                        } else {
//                            Glide.with(CarameAlbumActivity.this).load(imagePath).placeholder(R.mipmap.ic_launcher).error(R.mipmap.ic_launcher).into(shenfenfan);
                        }
                    }
